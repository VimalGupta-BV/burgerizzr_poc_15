...
class Warehouse(models.Model):
    ...
    def display_prod_stock(self, product_id):
        ...
        for warehouse_obj in self.search([('company_id', '=', self.env.user.company_id.id)]):
            ...
            location_list = []
            warehouse_total = []
            warehouse_total_qty = product.with_context({'warehouse': warehouse_obj.id})._compute_quantities_dict(
            ...
            for each in self.env['stock.location'].search([('location_id', 'child_of', parent_loc_id)]):
                available_qty = product.with_context(
                    {'location': each.id, 'compute_child': False})._compute_quantities_dict(self._context.get('lot_id'),
                                                                                            self._context.get(
                                                                                                'owner_id'),
                                                                                            self._context.get(
                                                                                                'package_id'),
            ...
            warehouse_list.append({'id': warehouse_obj.id, 'name': warehouse_obj.name, 'locations': location_list,
