...
class ResConfigSettingsWizard(models.TransientModel):
    ...
    def set_values(self):
        ...
        icp_sudo.set_param("stock.module_product_expiry", self.module_product_expiry)

    @api.model
    def get_values(self):
        res = super(ResConfigSettingsWizard, self).get_values()
        icp_sudo = self.env['ir.config_parameter'].sudo()
        module_product_expiry = icp_sudo.get_param('stock.module_product_expiry')
...
class ProductExpiryReport(models.Model):
    ...
    group_by = fields.Selection([('location', 'Location'), ('category', 'Category')],
                                string="Group By",
                                default="location")
    ...
    def print_pdf_report(self):
        if self.env['ir.config_parameter'].sudo().get_param('stock.module_product_expiry'):
            ...
            raise UserError(
                _('Please enable "Expiration Dates" from Inventory-->Settings-->Traceability'))
    ...
    def print_product_expiry_report(self, report_type):
        ...
        location_ids = self.location_ids.ids or self.env['stock.location'].search(
            [('usage', '=', 'internal')]).ids
        category_ids = self.category_ids.ids or self.env['product.category'].search([]).ids
        first_query = '''SELECT sq.location_id, sl.usage, spl.product_id, spl.id, 
                            spl.expiration_date,spl.name,pc.name as product_category,
        ...
        second_query = '''SELECT sq.location_id, sl.usage, spl.product_id, spl.id, 
                            spl.expiration_date,spl.name,pc.name as product_category,
        ...
        if len(res) == 0:
            ...
            if self.group_by == 'category':
                vals = {}
                for each in res:
                    if each.get('location_id') is None:
                        location_name = "--"
                    else:
                    ...
                    if each['product_category'] not in vals:
                        vals[each.get('product_category')] = [
                            {'name': each.get('name'),
                             ...
                             'remaining_days': relativedelta(each.get('expiration_date'),
                                                             date.today()).days,
                        ...
                        vals[location_name].append(
                            {'name': each.get('name'),
                             ...
                             'available_qty': self.env['stock.production.lot'].browse(
                                 each.get('id')).product_qty if each.get('id') else False, })
        vals.update(
            {'group_by': self.group_by, 'num_days': self.num_expiry_days,
             'today_date': date.today()})
        vals_new = {}
        vals_new.update({'stock': vals})
        if report_type == 'pdf':
            return self.env.ref('flexiretail_com_advance.product_expiry_report').report_action(
                self,
                data=vals_new)
        ...
        for key, value in vals.items():
            if vals.get('group_by') and key not in ['group_by', 'num_days', 'today_date']:
                ...
                if value not in [vals.get('num_day'), vals.get('today_date')]:
                    ...
                    for each in value:
                        count = 0
                        for key_each, val in each.items():
