...
class ReportGeneralLedgerPdf(models.AbstractModel):
    _name = 'report.flexibite_ee_advance.general_ledger_template'
    ...
    def _get_init_bal(self, from_date, company_id, account_id):
        ...
        if from_date and company_id and account_id:
            ...
            if account_config_id and account_config_id.fiscalyear_last_month and \
                    account_config_id.fiscalyear_last_day:
                fiscal_month = account_config_id.fiscalyear_last_month
                fiscal_end_date = account_config_id.fiscalyear_last_day
                if fiscal_month == 12:
                    current_year -= 1
                fiscal_year_start_date = str(current_year) + '-' + str(fiscal_month) + '-' + str(
                    fiscal_end_date)
                fiscal_year_start_date = datetime.strftime(
                    datetime.strptime(fiscal_year_start_date, '%Y-%m-%d') + timedelta(days=1),
                    '%Y-%m-%d')
            else:
        ...
        if result and result[0].get('debit') and result[0].get('credit'):
            result = [result[0].get('debit'), result[0].get('credit'),
                      result[0].get('debit') - result[0].get('credit')]
    ...
    def _get_report_values(self, docids, data=None):
        ...
        self.env.cr.execute(sql + where_clause + group_by_clause + order_by_clause)
