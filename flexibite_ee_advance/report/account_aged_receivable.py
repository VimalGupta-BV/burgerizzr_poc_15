...
class AgedReceivablePdf(models.AbstractModel):
    _name = 'report.flexibite_ee_advance.aged_receivable_template'
    ...
    def _get_partner_move_lines_custom(self, account_type, date_from, target_move, period_length):
        ...
        cr.execute(query, (
            ...
            tuple(company_ids)))
        aml_ids = cr.fetchall()
        aml_ids = aml_ids and [each[0] for each in aml_ids] or []
        ...
        for partner in partners:
            ...
            # Add for total
            total[(final_num + 1)] += values['total']
    ...
    def _get_report_values(self, docids, data=None):
        ...
        accont_moveline, total, dummy = self._get_partner_move_lines_custom(account_type, date_from,
                                                                            target_move,
                                                                            period_length)
        model = self.env.context.get('active_model')
