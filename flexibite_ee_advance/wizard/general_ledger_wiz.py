...
class GeneralLedgerReportWizard(models.TransientModel):
    ...
    sortby = fields.Selection(
        [('sort_date', 'Date'), ('sort_journal_partner', 'Journal & Partner')], string='Sort by',
        required=True, default='sort_date')
    journal_ids = fields.Many2many('account.journal', 'account_report_general_ledger_journal_rel',
                                   'account_id',
    ...
    def print_pdf(self, data=None):
        data = {
            ...
            'form': self.read(
                ['date_from', 'date_to', 'journal_ids', 'display_account', 'target_move',
                 'include_init_balance',
                 'sortby'])[0]}
        datas = {
            'ids': self._ids,
            'docs': self._ids,
        ...
        return self.env.ref('flexibite_ee_advance.report_general_ledger').report_action(
            self, data=datas)
    ...
    def print_xls(self, data=None):
        ...
        worksheet.write_merge(0, 2, 0, 8, company_name, style=style_border)
        ...
        worksheet.write_merge(5, 5, 0, 3,
                              ','.join(map(str, [each.code for each in self.journal_ids])),
                              style=fontbold)
        worksheet.write_merge(5, 5, 4, 5, display_acccount, style=fontbold)
        worksheet.write_merge(5, 5, 6, 8,
                              'All Posted Entries' if self.target_move == 'posted'
                              else 'All Entries',
                              style=fontbold)

        worksheet.write_merge(6, 6, 0, 2,
                              'Sort by : ' + 'Date' if self.sortby == 'sort_date'
                              else 'Sort by : ' + 'Journal & Partner')
        worksheet.write_merge(6, 6, 3, 5,
        ...
        report_obj = self.env['report.flexibite_ee_advance.general_ledger_template']
        data = {
            'ids': self.env.context.get('active_ids', []),
            'model': self.env.context.get('active_model', 'ir.ui.menu'),
            'form': self.read(
        ...
        report_data = report_obj.with_context(
            active_model='account.general.ledger.wiz')._get_report_values(self,
                                                                          data=datas)
        num = 9
        for each in report_data['Accounts']:
            worksheet.write_merge(num, num, 0, 5, each['code'] + ' ' + each['name'], fontbold)
            ...
            worksheet.write_merge(num, num, 7, 7, formatLang(
                self.env, float(each['credit']),
                currency_obj=self.env.user.company_id.currency_id),
                                  fontbold)
            worksheet.write_merge(num, num, 8, 8, formatLang(
                self.env, float(each['balance']),
                currency_obj=self.env.user.company_id.currency_id),
                                  fontbold)
            if self.include_init_balance:
                num = num + 1
                worksheet.write_merge(num, num, 4, 5, 'Initial Balance')
                worksheet.write_merge(num, num, 6, 6,
                                      formatLang(self.env, float(each['init_bal'][0]),
                                                 currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(num, num, 7, 7,
                                      formatLang(self.env, float(each['init_bal'][1]),
                                                 currency_obj=self.env.user.company_id.currency_id))
                worksheet.write_merge(num, num, 8, 8,
                                      formatLang(self.env, float(each['init_bal'][2]),
                                                 currency_obj=self.env.user.company_id.currency_id))
            num = num + 1
            for each_line in each['move_lines']:
                worksheet.write_merge(num, num, 0, 0, str(each_line['ldate']))
                worksheet.write_merge(num, num, 1, 1, each_line['lcode'])
                worksheet.write_merge(num, num, 2, 2, each_line['partner_name'])
                worksheet.write_merge(num, num, 3, 3, each_line['lref'])
                worksheet.write_merge(num, num, 4, 4, each_line['move_name'])
                worksheet.write_merge(num, num, 5, 5, each_line['lname'])
                worksheet.write_merge(num, num, 6, 6,
                                      formatLang(self.env, float(each_line['debit']),
                                                 currency_obj=self.env.user.company_id.currency_id),
                                      style_pc)
                worksheet.write_merge(num, num, 7, 7,
                                      formatLang(self.env, float(each_line['credit']),
                                                 currency_obj=self.env.user.company_id.currency_id),
                                      style_pc)
                worksheet.write_merge(num, num, 8, 8,
                                      formatLang(self.env, float(each_line['balance']),
                                                 currency_obj=self.env.user.company_id.currency_id),
                                      style_pc)
                num = num + 1
