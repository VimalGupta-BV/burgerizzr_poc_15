...
class NonMovingStock(models.TransientModel):
    ...
    report_type = fields.Selection([('pdf', 'PDF Report'), ('xls', 'Excel Report')],
                                   string="Report Type",
                                   required=True, default="pdf")
    state = fields.Selection([('new', 'New'), ('done', 'done'), ('sent', 'Sent')], string="State",
                             default='new')
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        res = super(NonMovingStock, self).fields_view_get(view_id=view_id, view_type=view_type,
                                                          toolbar=toolbar,
                                                          submenu=submenu)
        group_ids = ast.literal_eval('{}'.format(
    ...
    def print_report(self):
        ...
        if self.non_moving_product_days <= 0:
            raise UserError(_("Please Enter Days To Proceed."))
        ...
        for each in warehouse_ids_list:
            location_ids = location_obj.search(
                [('usage', '=', 'internal'), ('location_id', 'child_of',
            ...
            stock_quant_query = '''
                select sq.product_id,pt.default_code,sl.id,pt.name,pc.name as category,
                sq.quantity,sl.complete_name from stock_quant sq
                ...
                and sq.product_id %s
                 ''' % ("(%s)" % ",".join(map(str, sale_category_ids_list.ids)),
                        "(%s)" % ",".join(map(str, location_ids.ids)),
                        "not in (%s)" % ",".join(
                            map(str, product_list)) if product_list else "is not null"
                        )
            ...
            if self.group_by_category:
                for data in stock_quant_data:
                    ...
                    data.update(
                        {'sell_price': product.list_price, 'unit_price': product.standard_price})
        ...
        if not vals:
            raise UserError(_('NO Stock Found'))
        if self.report_type == 'pdf':
            if self.env.context.get('send_mail'):
                ...
                attachment_id = self.env['ir.attachment'].sudo().create(
                    {'datas': values, 'name': 'Non Moving Stock.pdf'})
                return attachment_id
            else:
                final_non_moving_data.update({'break': True})
                vals_new = {'products': final_non_moving_data}
                return self.env.ref(
                    'flexibite_ee_advance.non_moving_stock_report').report_action(self,
                                                                                   data=vals_new)
            ...
            worksheet.write_merge(3, 3, 0, 1,
                                  'Non Moving Product in Last ' + str(
                                      self.non_moving_product_days) + ' Days',
                                  bold)
            worksheet.write_merge(3, 3, 5, 5,
            ...
            if self.group_by_category:
                for rec in vals:
                    ...
                    worksheet.write(row, 1, rec)
                    row = row + 1
                    for num in range(len(list1)):
                        if list1[num] in ['Cost Price', 'Sale Price']:
                            worksheet.write(row, num, list1[
                                num] + "(" + self.env.user.company_id.currency_id.symbol + ")",
                                            style1)

                        elif list1[num] == 'Quantity':
                            worksheet.write(row, num, list1[num], style1)
                        else:
                            worksheet.write(row, num, list1[num], bold)
                    row = row + 1
                    for each in vals.get(rec):
                        worksheet.col(0).width = 4000
                        worksheet.write(row, 0, each.get('default_code'))
                        worksheet.col(1).width = 6000
                        worksheet.write(row, 1, each.get('name'))
                        worksheet.col(2).width = 8000
                        worksheet.write(row, 2, each.get('complete_name'))
                        worksheet.write(row, 3, each.get('quantity'), horz_style_pc)
                        worksheet.col(4).width = 4500
                        worksheet.write(row, 4, str('%.2f' % (each.get('unit_price'))) + str(
                            self.env.user.company_id.currency_id.symbol), horz_style_pc)
                        worksheet.col(5).width = 4500
                        worksheet.write(row, 5, str('%.2f' % (each.get('sell_price'))) + str(
        ...
        for each in groups_ids:
            ...
            try:
                template_id = self.env.ref(
                    'flexibite_ee_advance.send_mail_non_moving_stock_report')
            except Exception:
            ...
            if mail_id.state == 'sent':
                self.write(
                    {'send_mail_message': "Mail has been sent successfully!", 'state': 'sent'})
            else:
                self.write({'send_mail_message': "Error in sending mail, Please try again!",
                            'state': 'sent'})
            return {
                'name': 'Non Moving Stock',
                'view_type': 'form',
