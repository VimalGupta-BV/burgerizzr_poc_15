...
class AccountAgedReceivableReportPdf(models.TransientModel):
    ...
    def generate_aged_receivable(self):
        ...
        return self.env.ref('flexibite_ee_advance.report_aged_receivable').report_action(
            self, data=datas)

    def generate_account_receivable_xls(self):
        report_obj = self.env['report.flexibite_ee_advance.aged_receivable_template']
        ...
        font_p.bold = True
        style_p.font = font
        style_pc.alignment = alignment_rgt
        ...
        worksheet.write_merge(3, 3, 0, 7, 'Aged Receivable', style=style_border)
        ...
        worksheet.write_merge(5, 5, 1, 2, 'Receivable Accounts', style=style_border)
        ...
        accont_moveline, total, dummy = report_obj._get_partner_move_lines_custom(
            ['receivable'], str(self.start_date), self.target_move, self.period_length)
        if total:
