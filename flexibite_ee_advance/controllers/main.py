...
class CustomerDisplayController(BusController):
    ...
    def _poll(self, dbname, channels, last, options):
        ...
        """This code is for kitchen screen"""
...
class PosMirrorController(http.Controller):
    ...
    @http.route('/web/customer_display/<int:session_id>', type='http', auth='user')
    def white_board_web(self, session_id, **k):
        config_id = False
        pos_sessions = request.env['pos.session'].search([('id', '=', session_id), ('rescue', '=', False)])
        if pos_sessions:
