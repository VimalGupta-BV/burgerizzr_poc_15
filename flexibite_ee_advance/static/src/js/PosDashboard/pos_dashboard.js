odoo.define('flexibite_ee_advance.pos_dashboard', function (require) {
    "use strict";
    ...
    var POSDashboard = AbstractAction.extend({
        ...
        willStart:function(){
            ...
            var user_companies = []
            _.each(session.user_companies.allowed_companies, function(each){
                user_companies.push(each);
            });
            this.user_companies = user_companies;
            this.set_currency();

            self.session = session;
            var currency_set = this._rpc({
                model: 'res.company',
                method: 'search_read',
                args: [[['id', '=', this.session.company_id]], ['currency_id']],
            }, {async: false}).then(function (result) {
                if(result)
                    self.currency_id = result[0].currency_id[0];
            });
            return Promise.all([res, currency_set]);
        },
            ...
            this._rpc({
                ...
                if(res){
                    ...
                    if(res['login_user_img']){
                        self.$el.find('#image_viewer').html('<img class="rounded" style="width:70px" src="data:image/jpeg;base64,'+res['login_user_img']+'"/>');
                    }else{
                         self.$el.find('#image_viewer').html('<img class="rounded" src="flexibite_ee_advance/static/src/img/avtar.png">');
                    }
