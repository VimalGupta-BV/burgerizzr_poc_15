    ...
    DB.include({
        ...
        add_orders : function(orders){
            ...
            for(var i = 0, len = orders.length; i < len; i++){
                ...
                order['date_order'] =   moment(localTime).locale('en').format('YYYY-MM-DD hh:mm:ss')
        ...
        thousandsDecimalChanger: function(amount) {
            if(amount != 0){
                var converted_amount = amount.toString().replace(this.decimalSeparator(), '!').replace(this.thousandsSeparator(), '').replace('!', '.')
