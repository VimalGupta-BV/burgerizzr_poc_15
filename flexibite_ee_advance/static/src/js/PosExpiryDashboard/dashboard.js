    ...
    var ProductExpiryDashboard = AbstractAction.extend({
        ...
        location_expire: function(result){
            ...
            self.do_action({
                   ...
                   domain:[['expiry_state','=','near_expired'],['location_id','=',parseInt(result)]],
            });
        },
           ...
           .then(function(records){
               ...
               var userList = new List('warehouse', options, values);
                   ...
                   var links = Object.keys(records['day_wise_expire']).map(function(key) {
                        ...
                        if(records['day_wise_expire'][key]['color']){
                            ...
                            if(records['day_wise_expire'][key]['text_color']){
                                html += "<div class='info-box'> <span class='info-box-icon bg-aqua product_config' id='product_expiry_data' style='cursor:pointer;background-color:"+records['day_wise_expire'][key]['color']+" !important;color:"+records['day_wise_expire'][key]['text_color']+" !important;'>"+records['day_wise_expire'][key]['product_id'].length + "</span>";
                            }
                            else{
                   ...
                   $(document).ready(function () {
                       $('#expired_product').click(function(){
                            self.open_near_expire(records['expired_result'])
                        });

                        $('#near_expired').click(function(){
                            self.open_near_expire(records['category_near_expire'])
                        });
