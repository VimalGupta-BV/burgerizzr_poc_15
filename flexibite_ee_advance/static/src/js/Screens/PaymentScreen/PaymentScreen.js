odoo.define('flexibite_ee_advance.PaymentScreen', function (require) {
    ...
    const AsplRetPaymentScreenInh = (PaymentScreen) =>
        class extends PaymentScreen {
            constructor() {
                super(...arguments);
                this.state = useState({ remaining_wallet_amount: 0});
                ...
                // this.get_remaining_loyalty_point()
            }
            async redirectBack(){
            ...
            // async get_remaining_loyalty_point(){
            //     var self = this
            //     if (this.env.pos.get_order() && this.env.pos.get_order().get_client() && this.env.pos.get_order().get_client().id){
            //         await this.rpc({
            //             model: "res.partner",
            //             method: "search_read",
            //             domain: [['id', '=', this.env.pos.get_order().get_client().id]],
            //             fields:['remaining_points']
            //         }, {async: false}).then(function(results){
            //             _.each(results, function(result){
            //                 self.state.remaining_loyalty_point = result.remaining_points
            //             });
            //         });
            //     }
            // }
            ...
            //     var remaining_point = self.state.remaining_loyalty_point;
            //     if (remaining_point == 0){
            //         this.showNotification(`No Loyalty Points !!`);
            //         return
            //     }
            ...
            async validateOrder(isForceValidate)  {
                ...
                //     }
                // }
                if(this.currentOrder.get_change() && this.env.pos.config.enable_wallet && this.currentOrder.get_client()
                    && (this.currentOrder.get_type_for_wallet() != 'change')){
