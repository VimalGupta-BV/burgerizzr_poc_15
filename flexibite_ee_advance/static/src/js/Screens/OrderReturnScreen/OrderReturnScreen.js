odoo.define('flexibite_ee_advance.OrderReturnScreen', function(require) {
    'use strict';
        ...
        async _onSearch() {
            ...
            if (search_order_id && search_order_id.totalCount > 0){
                ...
                if(this.state.orderlines.length == 0){
                    this.state.orders = []
                    var $messageDiv = $('.return_msg_string');
                    $messageDiv.show().html('Already Returned Order !!');
                ...
                this.state.orders = []
                var $messageDiv = $('.return_msg_string');
                $messageDiv.show().html('No Any Order Found !!');
                setTimeout(function(){ $messageDiv.hide().html('');}, 2000);
            }
            this.render();
        }
            ...
            if (this.state.orderlines && this.state.orderlines.length > 0){
                ...
                _.each(this.state.orderlines, function (lines) {
                    if (lines.return_qty > 0){
                        var line_unit_price = lines.price_subtotal
                        if (lines.qty > 0){
                            line_unit_price = lines.price_subtotal / lines.qty
                        }else{
                            line_unit_price = lines.price_subtotal / 1
                        }
...
//    OrderReturnScreen.hideOrderSelector = true;
