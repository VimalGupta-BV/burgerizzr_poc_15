    ...
    class GiftCardScreen extends PosComponent {
        ...
        _getSearchFields() {
            const fields = {
                CARD_NUMBER: {
                    repr: (order) => order.card_no,
                    displayName: this.env._t('Card Number'),
                    modelField: 'card_no',
                },
                ISSUE_DATE: {
                    repr: (order) => moment(order.issue_date).locale('en').format('YYYY-MM-DD hh:mm A'),
                    displayName: this.env._t('Issue Date(YYYY-MM-DD hh:mm A)'),
                    modelField: 'issue_date',
                },
                EXPIRE_DATE: {
                    repr: (order) => moment(order.expire_date).locale('en').format('YYYY-MM-DD hh:mm A'),
                    displayName: this.env._t('Expire Date(YYYY-MM-DD hh:mm A)'),
                    modelField: 'expire_date',
                },
                CUSTOMER: {
                    repr: (order) => order.customer_id[1],
