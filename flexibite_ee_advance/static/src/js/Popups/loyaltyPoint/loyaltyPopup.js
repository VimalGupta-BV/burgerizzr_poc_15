odoo.define('flexibite_ee_advance.loyaltyPopup', function(require) {
    'use strict';
    ...
    class loyaltyPopup extends AbstractAwaitablePopup {
        constructor() {
            super(...arguments);
            this.state = useState({ LoyaltyPoints: 0.00,
                pointsAmount:0.0, LoyaltyValidation: true});
        }
        async onInputKeyUpValidation(e){
            if(this.state.LoyaltyPoints){
                if(Number(this.state.LoyaltyPoints) <= this.props.remaining_point){
                    $('.redeem_loyalty_input').css('border','');
                    this.state.LoyaltyValidation = true;
                } else {
                    $('.redeem_loyalty_input').css('border','1px solid red');
                    this.state.LoyaltyValidation = false;
                }
            }
        }
